...
public class PlayerController : EntityBehaviour<IPhysicState>
{
    ...
    private void Update()
    {
        // Entity can move, rotate and shoot only if it has control (IF IT IS THE OWNER)
        if (_hasControl)
            PollKeys();
    }
    ...
    /// <summary>
    /// Inputs the taken variables into the PlayerCommand 
    /// PlayerMotor is responsible for movement
    /// PlayerWeapons is responsible for shooting
    /// </summary>
    ...
    /// <summary>
    /// Upon Calling the Execute Command Functions, the position and rotation will be sent to the server
    /// </summary>
